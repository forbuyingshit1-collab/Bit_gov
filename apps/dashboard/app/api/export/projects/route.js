...
export async function GET(request) {
  ...
  const target = new URL(sourceUrl.searchParams.get('dataset') === 'company' ? '/v1/export/company.csv' : '/v1/export/projects.csv', process.env.BIT_GOV_API_URL);
  for (const [key, value] of sourceUrl.searchParams) target.searchParams.append(key, value);
  const response = await fetch(target, { cache: "no-store" });
  if (!response.ok) return Response.json(await response.json().catch(() => ({ error: 'export_unavailable' })), { status: response.status });
